...
The SDK enables you to easily use the Infinario's powerful tracking capabilities in your Marmalade application.

Make sure to checkout the `InfinarioSDK.mkb` Marmalade project file in this repo. When loaded by Marmalade Hub it can be easily used to test the SDK's functionality. Also you should take a short look at the file `Test.cpp` as it contains some useful examples of how to use the SDK. More information on how to run the test project can be found at the bottom of this readme.

Tested on Marmalade v8.0.0.

##Quick Start

To start tracking all you need to do is to add the contents of the `src` folder into your project, and include the header file `Infinario.h` in any place you wish to track player events.
...
Here are a few examples you could track events:
...
The first argument of the `track()` method is the tracked event's name, the second are the tracked event's attributes. The attributes must be a valid JSON string.
...
Each instance of the Infinario class maintains an internal queue of pending requests. Whenever one of the methods `track()`, `identify()` or `update()` is called the request is added to the end of the queue. The Infinario class processes one request at a time, so that a new request is sent only when the last one was finalized (meaning we either got a response or the request failed for some reason) or there are no requests waiting to be sent.
...
// Create an instance of the Infinario class, which internally generates a 'customerCookie' for anonymous player identification.
// The 'customerCookie' is a random string generated using the device's hardware and running OS.
...
```
// Now all the data tracked so far will belong to the player who's unique id is 'infinario@example.com'
infinario.identify("infinario@example.com");
```

The only way we could know that the identify method succeeded is when we receive a confirmation from the Infinario server. That is why the `customerId` we gave to the `identify()` method will be assigned to events that tracked after we received the Infinario server's response. Don't worry, all requests sent between calling the `identify()` method and us receiving the Infinario server's confirmation are handled correctly.

It is strongly recommended to call the `identify()` method at most once and only on instances of the Infinario class that were constructed without a `customerId`. The `identify()` method cannot be used to change an existing player's `customerId`.
...
The first argument are the player's new attributes which will be merged with any existing attributes. The attributes must be a valid JSON string.
...
Since requests are proccessed asynchronously, user-defined callback functions provide a way to react to when a response arrives from the server.

A general note of caustion. When passing custom data to these functions or when using external data within them, make sure that the variables remain valid (ie. the underlying memory does not get deallocated) before the callback function gets called.

###Response Callbacks

Each of the methods that need to communicate with the Infinario server (`identify()`, `update()` and `track()`), has two optional parameters: a callback function pointer and a pointer to custom data that are passed to the callback function when it is called. The callback function is called after a response is successfully received from the server or if an error occurs while processing the request.
    ...
    const std::string &responseBody, void *)
{
    if (responseStatus == Infinario::ResponseStatus::Success) {
        // Prints the received JSON response from the Infinario server, if all went well.
...
You can see the callback functions are given 5 arguments:
* `httpClient` - a reference to the object used internally by the Infinario class to send all the requests. Detailed information about the request can be obtained by querying this object. Useful for debugging purposes.
* `requestBody` - the full HTTP request body sent by the Infinario SDK to the Infinario server. Useful for debugging purposes.
...
As you can see the Empty Request Queue Callback has one argument and that's the data we gave it when we assigned the callback by calling the `SetEmptyRequestQueueCallback()` method.
...
From now on all requests will be sent through the supplied proxy server. To send requests directly again just call:
...
The file `InfinarioSDK.mkb` file can be used to setup a Marmalade project, all you need to do is open it in Marmalade Hub. Before running the project make sure to setup the following preprocessor definitions in `Test.cpp`:
...
If everything is setup correctly you should see only green successful tests.

##Additional Notes

For the most accurate information (like exact method prototypes and some helpful information on how to use the SDK's methods) be sure to take a look at the file `src/Infinario.h`.
